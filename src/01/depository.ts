...
import { Num, ZERO_ONE_DEVNET_PROGRAM_ID, ZERO_ONE_MAINNET_PROGRAM_ID, ZO_DEVNET_STATE_KEY, ZO_FUTURE_TAKER_FEE, ZO_MAINNET_STATE_KEY } from '@zero_one/client';
...
export class ZoDepository {
    ...
    public info() {
        console.groupCollapsed("[Depository debug info - Collateral mint:", this.collateralMintSymbol, " - decimals", this.collateralMintDecimals, "]");
        console.table({
    ...
    public async getOnchainAccount(connection: Connection, options?: ConfirmOptions): Promise<ZoDepositoryAccount> {
        ...
        return uxdProgram.coder.accounts.decode("zoDepository", ai.data) as ZoDepositoryAccount;
    }
    ...
    getPerpPriceNum(zo: Zo): Num {
        const pmi = zo.state.getMarketIndexBySymbol(this.collateralMintSymbol + "-PERP");
    ...
    getLimitPrice(slippage: number, perpOrderTakerSide: 'short' | 'long', zo: Zo): number {
    ...
    async getZoAccountCollateralOpenOrdersAccountKey(zo: Zo, controlKey: PublicKey): Promise<PublicKey> {
    ...
    public async settleMarginAccountFunds(zo: Zo): Promise<string> {
        let zoAccount = await zo.loadMarginAccount(this.zoAccountPda);
